				...
				isOpen && (
					<>
						<motion.div
							...
							className="fixed top-0 left-0 z-[100] h-full w-full cursor-pointer bg-black/70" onClick={closeModal}
						/>
						...
						<motion.div
							...
							className="-translate-x-1/2 -translate-y-1/2 fixed top-1/2 left-1/2 z-[110] rounded-lg bg-slate-900"
						>
							<div className="relative flex h-full max-h-full min-h-96 w-[26rem] flex-col gap-y-3 px-8 py-4">
								<button type="button" onClick={closeModal} className="absolute top-3 right-4 text-lg text-white/40">
									<FontAwesomeIcon icon={faXmark} className="p-1" />
								</button>

								<div className="flex flex-col items-center justify-center pt-8 pb-2">
									<div>
										<FontAwesomeIcon icon={faHeartRegular} className="-rotate-45 text-pink-600 text-xl" />
										<FontAwesomeIcon icon={faHeartRegular} className="text-3xl text-pink-600" />
										<FontAwesomeIcon icon={faHeartRegular} className="rotate-45 text-pink-600 text-xl" />
									</div>
									<h2 className="font-bold text-2xl text-white">Sponsor this project</h2>
								</div>
								...
								<div className="mt-auto flex h-full flex-col items-center justify-between space-y-4 py-2">
									...
									<div>
										<button type="button" onClick={closeModal} className="rounded-md px-4 py-2 text-sm text-white/50 transition-colors hover:bg-white/5">
