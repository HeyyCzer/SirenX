...
export const importXmlFile = async (fileContent: string) => {
	...
	const sirens = json?.CVehicleModelInfoVarGlobal?.Sirens?.Item;
	if (!sirens) {
		return void Modal.fire({
			icon: "error",
			title: "Error",
	...
	if (sirensArray.length > 1) {
		...
		try {
			sirenOptions = sirensArray.reduce(
				(acc, siren) => ({
					...acc,
					[siren.id.$.value]:
						`${siren?.name?._text || "NO-NAME"} (ID: ${siren.id.$.value})`,
				}),
		...
		await Modal.fire({
			...
			selectedSiren = sirensArray.find(
				(siren) => siren.id.$.value === result.value,
			);
		});
	}
	...
	try {
		...
		void Modal.fire({
			...
			text:
				...
				"Failed to load your file. Are you sure this is a valid carcols.meta file?",
		});
	}
};
...
export const exportXmlFile = (editor: any, settings: any, fileName: string) => {
	...
	try {
		...
		Sentry.captureMessage(
			`Error while exporting file: ${(err as { customMessage?: string }).customMessage || "No message"}`,
			{ level: "warning" },
		);
		...
		return void Modal.fire({
			...
			text:
				(err as { customMessage?: string }).customMessage ||
				"Error while trying to export the file. Please, try again or reset the editor.",
		});
	}
};
